...
DROP TABLE IF EXISTS `service_handler_config`;
CREATE TABLE `service_handler_config`  (
   `id` bigint(20) NOT NULL AUTO_INCREMENT,
   `request_url` varchar(1024) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '接口地址',
   `request_method` varchar(8) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '请求类型post-get',
   `protocol` varchar(16) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '协议http-https',
   `category` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '类别',
   `invoking` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '方法',
   `asyn_type` bigint(2) NULL DEFAULT 0 COMMENT '异步调用 0-同步调用； 1-使用异步调用',
   `status` bigint(2) NOT NULL DEFAULT 0 COMMENT '状态，0-启用；1-禁用',
